...
func (b *Battlefield) PlaceUnit(unit *GameUnit, position domain.Position) *domain.ActionResult {
	...
	unitIsNotPlaced := b.FindUnit(unit.Uid) == nil
	if unitAtPosition := b.FindUnitByPosition(position); unitAtPosition != nil {
		if unitIsNotPlaced || unitAtPosition.PlayerInfo != nil && unitAtPosition.PlayerInfo.IsReady {
...
func (b *Battlefield) RemoveUnit(uid uint) {
	unit := b.FindUnit(uid)
	if unit == nil {
		return
	}
...
func (b *Battlefield) MoveToCorpses(uid uint) {
	...
	unit.IsDead = true
	unit.Position = domain.PositionOutOfBounds
	b.Units = survivedUnits
